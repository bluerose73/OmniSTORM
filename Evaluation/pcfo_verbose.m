...
function [gain, offset] = pcfo_verbose(in, kthres, RNStd, AvoidCross, doPlot, Tiles)
...
disp("TotalVar = " + mat2str(TotalVar));
disp("TotalInt = " + mat2str(TotalInt));
...
disp("Voffset = " + string(Voffset));
disp("slope = " + string(slope));
disp("raw_offset = " + string(offset));
disp("Variance of the fit (vv) = " + mat2str(vv));
...
disp("Computed total variance (AllTotalVar): " + string(AllTotalVar));
disp("Computed total intensity (AllTotalInt): " + string(AllTotalInt));
disp("Estimated gain: " + string(gain));
disp("Final offset after accounting for readout noise: " + string(offset));
